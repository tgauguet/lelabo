...
class UsersController < ApplicationController
				...
				format.html {
					if @user.account_name? && !@user.firstname?
				      redirect_to informations_de_votre_compte_path
					      ...
					      CreateIngredientsBaseJob.perform_later(@user) #delay
								CreateProvidersBaseJob.perform_later(@user) #delay
								CreateBaseRecipeIngredientsJob.perform_later(@user) #delay
	...
	def user_params
		accessible = [:name, :email, :stripe_customer_id, :country, :avatar, :firstname, :company, :job_type, :phone, :company_image, :phone, :skype, :profile_image, :account_name, :terms]
